...
window.onload = function() {
  ...
  fetch('/get-state')
    ...
    .then(data => {
      ...
      const [led, motorBF, motorLR,pinMotor1,pinMotor2,ssid] = data.split(',').map();
...
// Convertit un nom de broche (ex: "A1") en numéro de GPIO
...
// Exemple d'utilisation dans updatePinMotor
  function updatePinMotor() {
    const pinName1 = document.getElementById("pinMotor1").value; // Ex: "A1"
    const pinName2 = document.getElementById("pinMotor2").value; // Ex: "A2"
...
// Envoie les deux broches en tant qu'arguments séparés
    fetch(`/update?pinMotor1=${encodeURIComponent(gpio1)}&pinMotor2=${encodeURIComponent(gpio2)}`)
        .then(response => {
            if (response.ok) {
