...
// =================== Bibliothèques =========================
#include <WiFi.h>          // Remplace ESP8266WiFi.h
#include <WebServer.h>     // Remplace ESP8266WebServer.h
#include <DNSServer.h>
#include <EEPROM.h>
...
#define LED_PIN 8            // Broche de la LED intégrée (NodeMCU)
#define A1 1                  // Broche D1 pour les servos
#define A2 2                  // Broche D2 pour les servos
...
void setDefaultConfig() {
  clearEEPROM();
  //petitbot V4.6
  strcpy(config.ssid, "Petitbot N°esp32");  // SSID par défaut
  ...
  config.motorsFBInverted = false;      // Pas d'inversion avant/arrière
  config.pinMotor1 = 1;
  config.pinMotor2 = 2;
  pin_servoG = config.pinMotor1;
  pin_servoD = config.pinMotor2;
...
void setWifi() {
  ...
  WiFi.softAP(config.ssid, NULL, 0, false, 1);  // Crée un réseau avec le SSID configuré
...
void handleStopMotors() {
  servoG.attach(pin_servoG);
  servoD.attach(pin_servoD);
  servoG.detach(); // Détache le servo gauche
  servoD.detach(); // Détache le servo droit
...
void setup() {
  ...
  server.on("/update", handleUpdateConfig);
