...
void setup() {
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest * request) {
    request->redirect("/telecommande.html");
    delay(100);
  });
...
void read_config() {
  ...
  if (!SPIFFS.exists("/config.txt")) {
    Serial.println("Fichier /config.txt manquant !");
    return;
  }
...
void set_wifi() {
  /*
  Serial.println("=== Mode forcé : Point d'accès PetitBot ===");
  WiFi.mode(WIFI_AP);
  WiFi.softAP("PetitBot", "");
  Serial.println("Connectez-vous au WiFi 'PetitBot' et ouvrez http://192.168.4.1");
 */
